...
class UyuniSession:
    ...
    async def __aenter__(self):
        if self._client is not None:
            raise APIError("UyuniSession is already open")
    ...
    async def request(self, method: Literal["GET", "POST"], path: str,
                      context: str, *, params=None, json_body=None):
        """Send through the active authenticated client; adapters never log in."""
        if self._client is None:
            raise APIError("UyuniSession must be open before making API requests")
        return await call(self._client, method, path, context, params=params,
                          json_body=json_body, perform_login=False)

    async def get(self, path: str, context: str, *, params=None,
                  response_type: type = list, item_type: type = dict):
        """Read a typed Uyuni response and reject malformed collections."""
        result = await self.request("GET", path, context, params=params)
        if not isinstance(result, response_type):
            raise UnexpectedResponse(path, f"Expected {response_type.__name__} response")
        if response_type is list and any(not isinstance(item, item_type) for item in result):
            raise UnexpectedResponse(path, f"Expected a list of {item_type.__name__} values")
        return result
